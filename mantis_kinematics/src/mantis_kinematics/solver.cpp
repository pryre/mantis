...
bool MantisSolver::check_description( void ) {
	...
	if(!success) {
		ROS_ERROR_THROTTLE(2.0, "MantisSolver::check_description(): Unable to validate params and state");
	}
...
bool MantisSolver::check_state( void ) {
	...
	//Ensure the state are is ready, and that it matches the parameter configuration
	if( s_.ok() ) {
		//Make sure our local variables are up to date
	...
	if( check_description() ) {
		...
		if( ( ua.size() == qd.size() ) && ( tau.size() == qd.size() ) && ( num_states() == qd.size() ) ) {
			...
			if( calculate_mass_matrix(D) && calculate_coriolis_matrix(C) && calculate_loss_matrix(L) ) {
				...
				ROS_ERROR_THROTTLE(2.0, "MantisSolver::solve_inverse_dynamics(): Could not calculate dynamics matricies");
			}
		} else {
			ROS_ERROR_THROTTLE(2.0, "MantisSolver::solve_inverse_dynamics(): Vector lengths inconsistent");
		}
	}
...
bool MantisSolver::calculate_thrust_coeffs( double &kT, double &ktx, double &kty, double &ktz) {
	...
	if( check_parameters() ) {
		...
		if(p_.airframe_type() == "quad_x4") {
			...
			ROS_ERROR_THROTTLE(2.0, "MantisSolver::calculate_thrust_coeffs(): unknown mixer type");
		}
	}
