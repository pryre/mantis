...
#include <mantis_guidance_full/JointMovementAction.h>
...
double Joint::get_rd( void ) {
	return last_velocity_;
}

void Joint::set_action_goal( void ) {
	mantis_guidance_full::JointMovementGoal goal = *( as_.acceptNewGoal() );

	if ( ( goal.duration > ros::Duration( 0 ) ) && ( goal.positions.size() >= 2 ) ) {
		...
		ROS_ASSERT_MSG( ctrlp.size() >= 4, "Number of control points is <4 (%i)",
			(int)ctrlp.size() );
		...
		ROS_INFO( "Router %s: creating spline connecting %i points", name_.c_str(),
			(int)goal.positions.size() );
	} else {
		spline_in_progress_ = false;
		as_.setAborted();

		ROS_ERROR( "Router %s: at least 2 positions must be specified (%i), and "
				   "duration must be >0 (%0.4f)",
			name_.c_str(), (int)goal.positions.size(), goal.duration.toSec() );
	}
}

void Joint::get_spline_reference( double& pos, double& vel,
	const double u ) const {
	// x values need to be scaled down in extraction as well.
	ROS_ASSERT_MSG(
		( u >= 0.0 ) && ( u <= 1.0 ),
		"Invalid time point given for spline interpolation (0.0 <= t <= 1.0)" );
	...
	if ( !use_dirty_derivative_ ) {
		...
		// XXX: Manually derrive over a short period as proper derivative can't be
	...
	if ( has_reference() ) {
		...
		if ( spline_in_progress_ ) {
			if ( tc < spline_start_ ) {
				...
				mantis_guidance_full::JointMovementFeedback feedback;
				...
				double t_norm = normalize( ( tc - spline_start_ ).toSec(), 0.0,
					spline_duration_.toSec() );
				double npos = 0.0;
				double nvel = 0.0;
				...
				mantis_guidance_full::JointMovementResult result;
			...
			last_velocity_ = msg_out.velocity;
		} else {
			last_velocity_ = 0.0;

			msg_out.position = last_position_;
			msg_out.velocity = last_velocity_;
		}
