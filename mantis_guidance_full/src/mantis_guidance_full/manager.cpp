...
Manager::Manager()
	...
	dyncfg_control_settings_.setCallback(
		boost::bind( &MantisGuidanceFull::Manager::callback_cfg_control_settings,
			this, _1, _2 ) );
	...
	if ( p_.wait_for_params() && s_.wait_for_state() ) {
		...
		timer_ = nhp_.createTimer( ros::Duration( 1.0 / param_update_rate_ ),
			&MantisGuidanceFull::Manager::callback_timer, this );

		ROS_INFO_ONCE( "Aerial manipulator guidance loaded!" );
	} else {
		ROS_WARN( "Aerial manipulator guidance shutting down." );
		ros::shutdown();
	}
}
...
void Manager::callback_cfg_control_settings(
	mantis_guidance_full::ControlParamsConfig& config, uint32_t level ) {
...
void Manager::configure_joint_routers( void ) {
	...
	ROS_INFO_STREAM( "Spawned guidance for " << joint_routers_.size()
											 << " joints" );
}
...
Eigen::Affine3d Manager::calc_goal_base_transform( const Eigen::Affine3d& ge_sp,
...
Eigen::Vector3d Manager::calc_goal_base_velocity( const Eigen::Vector3d& gev_sp,
