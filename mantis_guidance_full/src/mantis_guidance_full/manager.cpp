...
void Manager::callback_timer(const ros::TimerEvent& e) {
	...
	if( ( p_.ok() ) && ( s_.ok() ) && ( contrail_.has_reference(e.current_real) ) ) {
		...
		ROS_ASSERT_MSG(joint_routers_.size() == p_.get_dynamic_joint_num(), "Joint guidance and dynamic joint param inconsistent (%i!=%i)", (int)joint_routers_.size(), (int)p_.get_dynamic_joint_num());
