...
public:
	BatonDetector()
		...
		nhp_.param( "camera_namespace", param_camera_namespace_,
			param_camera_namespace_ );

		// Subscribe to input video feed and publish output video feed
		sub_camera_info_ = nh_.subscribe<sensor_msgs::CameraInfo>(
			param_camera_namespace_ + "/camera_info", 100,
			&BatonDetector::camera_info_cb, this );
		...
		nhp_.param( "baton_description/length", param_baton_length_,
			param_baton_length_ );
		nhp_.param( "baton_description/radius", param_baton_radius_,
			param_baton_radius_ );
		nhp_.param( "baton_description/hsv_min/hue", param_hsv_min_hue_,
			param_hsv_min_hue_ );
		nhp_.param( "baton_description/hsv_min/sat", param_hsv_min_sat_,
			param_hsv_min_sat_ );
		nhp_.param( "baton_description/hsv_min/val", param_hsv_min_val_,
			param_hsv_min_val_ );
		nhp_.param( "baton_description/hsv_max/hue", param_hsv_max_hue_,
			param_hsv_max_hue_ );
		nhp_.param( "baton_description/hsv_max/sat", param_hsv_max_sat_,
			param_hsv_max_sat_ );
		nhp_.param( "baton_description/hsv_max/val", param_hsv_max_val_,
			param_hsv_max_val_ );

		nhp_.param( "hough_detector/accum_scale", param_hough_accum_,
			param_hough_accum_ );
		nhp_.param( "hough_detector/min_dist_scale", param_hough_min_dist_,
			param_hough_min_dist_ );
		nhp_.param( "hough_detector/param1", param_hough_param1_,
			param_hough_param1_ );
		nhp_.param( "hough_detector/param2", param_hough_param2_,
			param_hough_param2_ );
		nhp_.param( "hough_detector/min_radius", param_hough_min_rad_,
			param_hough_min_rad_ );
		nhp_.param( "hough_detector/max_radius", param_hough_max_rad_,
			param_hough_max_rad_ );

		nhp_.param( "filters/openning/size", param_morph_open_size_,
			param_morph_open_size_ );
		nhp_.param( "filters/closing/size", param_morph_close_size_,
		...
		nhp_.param( "filters/pose_low_pass/position_alpha", param_low_pass_pos_a_,
			param_low_pass_pos_a_ );
		nhp_.param( "filters/pose_low_pass/size_alpha", param_low_pass_size_a_,
			param_low_pass_size_a_ );

		model_points_.push_back( cv::Point3d( 0.0, 0.0, 0.0 ) ); // Center
		model_points_.push_back( cv::Point3d( 0.0, param_baton_radius_,
			param_baton_length_ / 2 ) ); // Top Left
		model_points_.push_back( cv::Point3d( 0.0, -param_baton_radius_,
			param_baton_length_ / 2 ) ); // Top Right
		model_points_.push_back( cv::Point3d(
			0.0, param_baton_radius_, -param_baton_length_ / 2 ) ); // Bottom Left
		model_points_.push_back( cv::Point3d(
			0.0, -param_baton_radius_, -param_baton_length_ / 2 ) ); // Bottom Right
		...
		sub_image_ = it_.subscribe( param_camera_namespace_ + "/image_raw", 100,
			&BatonDetector::image_cb, this );
	...
	void image_cb( const sensor_msgs::Image::ConstPtr& msg_in ) {
		...
		// Find contours
		cv::findContours( mask_image, contours, hierarchy, cv::RETR_TREE,
		...
		for ( int i = 0; i < polys.size(); i++ ) {
			...
			if ( polys[i].size() == 4 ) {
				...
				// Filter reading
				/*
        detected_ball_filtered_[0] = detected_ball_filtered_[0] - (
        param_low_pass_pos_a_ * ( detected_ball_filtered_[0] -
        detected_ball_raw[0] ) ) ;
        detected_ball_filtered_[1] = detected_ball_filtered_[1] - (
        param_low_pass_pos_a_ * ( detected_ball_filtered_[1] -
        detected_ball_raw[1] ) ) ;
        detected_ball_filtered_[2] = detected_ball_filtered_[2] - (
        param_low_pass_size_a_ * ( detected_ball_filtered_[2] -
        detected_ball_raw[2] ) ) ;


        //TODO: Should the be rounded?
        double p_x = std::round( detected_ball_filtered_[0] );	//Center
        of circle X
        double p_y = std::round( detected_ball_filtered_[1] );	//Center
        of circle Y
        double p_d = std::round( detected_ball_filtered_[2] );	//Radius
        of circle
        */
                ...
                image_points.push_back( cv::Point2d( p_x - p_d, p_y + p_d ) );
           //Top Left
                image_points.push_back( cv::Point2d( p_x + p_d, p_y + p_d ) );
           //Top Right
                image_points.push_back( cv::Point2d( p_x - p_d, p_y - p_d ) );
           ...
           //Bottom Right
                ...
                //We don't have any orientation data about the ball, so don't
           even bother
        ...
        */
		...
		if ( pub_debug_image_.getNumSubscribers() > 0 ) {
			...
			pub_debug_image_.publish(
				cv_bridge::CvImage( msg_in->header, "mono8", mask_image ).toImageMsg() );
		}
		...
		if ( pub_overlay_image_.getNumSubscribers() > 0 ) {
			...
			for ( int i = 0; i < contours.size(); i++ ) {
				drawContours( cv_ptr->image, polys, i, cv::Scalar( 0, 0, 255 ), 2, 4,
      ...
      std::round( found_circles[current_circle][1] ) );
              ...
              cv::circle( cv_ptr->image, center, radius, cv::Scalar( 0, 255, 0
      ), 5 );
      }

      //Red for detected position and size
      cv::Point raw_center( std::round( detected_ball_raw[0] ), std::round(
      detected_ball_raw[1] ) );
      ...
      cv::circle( cv_ptr->image, filtered_center, filtered_radius, cv::Scalar(
      0, 0, 255 ), 5 );
      */
