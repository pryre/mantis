	...
	if ( p_.wait_for_params() ) {
		if ( param_rate_ > 0.0 ) {
			...
			sub_state_odom_ = nh_.subscribe<nav_msgs::Odometry>(
				"state/odom", 10, &MantisStateServer::callback_state_odom, this );
			sub_state_battery_ = nh_.subscribe<sensor_msgs::BatteryState>(
				"state/battery", 10, &MantisStateServer::callback_state_battery,
				this );
			sub_state_joints_ = nh_.subscribe<sensor_msgs::JointState>(
				"state/joints", 10, &MantisStateServer::callback_state_joints, this );
			sub_state_imu_ = nh_.subscribe<sensor_msgs::Imu>(
				"state/imu_data", 10, &MantisStateServer::callback_state_imu, this );
			sub_state_mav_ = nh_.subscribe<mavros_msgs::State>(
				"state/mav_state", 10, &MantisStateServer::callback_state_mav, this );

			pub_state_ = nh_.advertise<mantis_msgs::State>( "state", 10 );
			timer_estimator_ = nhp_.createTimer( ros::Duration( 1.0 / param_rate_ ),
				&MantisStateServer::callback_estimator, this );
...
void MantisStateServer::callback_estimator( const ros::TimerEvent& e ) {
	if ( ( msg_odom_tr_ != ros::Time( 0 ) ) && ( msg_battery_tr_ != ros::Time( 0 ) ) && ( msg_joints_tr_ != ros::Time( 0 ) ) && ( msg_imu_tr_ != ros::Time( 0 ) ) && ( msg_mav_state_tr_ != ros::Time( 0 ) ) && ( p_.get_dynamic_joint_num() == r_.size() ) ) {
...
void MantisStateServer::callback_state_odom(
...
void MantisStateServer::callback_state_battery(
...
void MantisStateServer::callback_state_imu(
...
void MantisStateServer::callback_state_mav(
...
void MantisStateServer::callback_state_joints(
