...
void Joint::set_action_goal( void ) {
	mantis_router_full::JointMovementGoal goal = *(as_.acceptNewGoal());
	...
	if( spline_start_ > ros::Time(0) ) {
		...
		if( spline_in_progress_ ) {
			if( tc < spline_start_ ) {
				...
				mantis_router_full::JointMovementFeedback feedback;
				...
				mantis_router_full::JointMovementResult result;
