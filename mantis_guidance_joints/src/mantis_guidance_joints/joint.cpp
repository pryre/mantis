...
void Joint::set_action_goal( void ) {
	mantis_guidance_joints::JointMovementGoal goal = *( as_.acceptNewGoal() );
	...
	if ( spline_start_ > ros::Time( 0 ) ) {
		...
		if ( spline_in_progress_ ) {
			if ( tc < spline_start_ ) {
				...
				mantis_guidance_joints::JointMovementFeedback feedback;
				...
				mantis_guidance_joints::JointMovementResult result;
