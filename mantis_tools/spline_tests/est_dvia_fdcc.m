function [vias] = est_dvia_fdcc(v, tvec, conditioning)
...
%   Detailed explanation goes here
    ...
    for i = 2:np-1
        trim_kernel = kernel( kernel+i > 0 & kernel+i <= np );
        select_kernel = trim_kernel + i;
        time_kernel = tvec(select_kernel);
        dt = max(abs(time_kernel - tvec(i)));
        k = (time_kernel - tvec(i))/dt;
