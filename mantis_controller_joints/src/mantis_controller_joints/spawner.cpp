...
Spawner::Spawner() :
	...
	ROS_INFO("Joint position controller started, waiting for param definitions...");
...
void Spawner::configure_controllers( void ) {
	...
	for(int i = 0; i < p_.get_joint_num(); i++) {
		if( p_.joint(i).type != "static" ) {
			controllers_[cc] = new MantisControllerJoints::Controller(nhp_, p_.joint(i).name, traj_timeout_);
			cc++;
		}
	}
