...
MantisRouterJoints::MantisRouterJoints() :
	...
	nhp_("~"),
	p_(nh_),
	tracked_input_(MantisRouterJoints::LastJointReference::Undefined),
	...
	if( p_.wait_for_params() ) {
		...
		timer_joints_ = nhp_.createTimer(ros::Duration(1.0/param_update_rate_), &MantisRouterJoints::callback_joints, this );
...
MantisRouterJoints::~MantisRouterJoints( void ) {
}

void MantisRouterJoints::configure_publishers( void ) {
	if(tracked_input_ != MantisRouterJoints::LastJointReference::Undefined ) {
...
void MantisRouterJoints::callback_joints(const ros::TimerEvent& e) {
	...
	for(int i=0; i<p_.get_dynamic_joint_num(); i++) {
		...
		if(tracked_input_ == MantisRouterJoints::LastJointReference::DirectInput) {
			success = get_direct_input_reference(msg_out.position, msg_out.velocity, i);
		} else if(tracked_input_ == MantisRouterJoints::LastJointReference::DiscreteProgress) {
			success = get_discrete_progress_reference(msg_out.position, msg_out.velocity, i);
		} else if(tracked_input_ == MantisRouterJoints::LastJointReference::CubicSpline) {
...
void MantisRouterJoints::callback_cfg_joint_goals(mantis_router_joints::JointGoalsConfig &config, uint32_t level) {
	...
	tracked_input_ = MantisRouterJoints::LastJointReference::DirectInput;
}

bool MantisRouterJoints::get_direct_input_reference( double &pos_ref, double &vel_ref, const unsigned int index ) {
	bool success = false;
...
bool MantisRouterJoints::get_discrete_progress_reference( double &pos_ref, double &vel_ref, const unsigned int index ) {
	bool success = false;
...
bool MantisRouterJoints::get_cubic_spline_reference( double &pos_ref, double &vel_ref, const unsigned int index, const ros::Time& tc ) {
	bool success = false;
...
void MantisRouterJoints::callback_discrete_progress( const contrail_msgs::DiscreteProgress::ConstPtr& msg_in ) {
	...
	tracked_input_ = MantisRouterJoints::LastJointReference::DiscreteProgress;
}

void MantisRouterJoints::callback_cubic_spline( const contrail_msgs::CubicSpline::ConstPtr& msg_in ) {
	...
	if( check_msg_spline(*msg_in, tc) ) {
		...
		if( ( start_time + msg_in->duration ) > tc ) {
			...
			tracked_input_ = MantisRouterJoints::LastJointReference::CubicSpline;
		} else {
...
void MantisRouterJoints::callback_joint_list( const mantis_msgs::JointTrajectoryList::ConstPtr& msg_in ) {
...
void MantisRouterJoints::generate_splines( void ) {
...
bool MantisRouterJoints::check_msg_spline(const contrail_msgs::CubicSpline& spline, const ros::Time t ) {
