...
Manager::Manager() :
	...
	ROS_INFO("Joint router started, waiting for param definitions...");

	if( p_.wait_for_params() ) {
		configure_routers();
		timer_ = nhp_.createTimer(ros::Duration( 1.0 / param_update_rate_ ), &Manager::callback_timer, this );
	} else {
...
void Manager::configure_routers( void ) {
	...
	for(int i = 0; i < p_.get_joint_num(); i++) {
		if( p_.joint(i).type != "static" ) {
			routers_[cc] = new MantisRouterJoints::Joint(nhp_, p_.joint(i).name);
			cc++;
		}
	}

	ROS_INFO_STREAM("Spawned " << routers_.size() << " joint routers");
}
