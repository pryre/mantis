...
ControllerAcro::ControllerAcro() :
	...
	sub_actuator_control_ = nh_.subscribe<mavros_msgs::ActuatorControl>("command/force_compensation", 1, &ControllerAcro::callback_force_comp, this);
	...
	if(p_.wait_for_params()) {
		tmr_rc_out_ = nhp_.createTimer(ros::Duration(1 / pwm_update_rate_), &ControllerAcro::callback_mixer, this);
...
int32_t ControllerAcro::int32_constrain(const int32_t i, const int32_t min, const int32_t max) {
	return (i < min) ? min : (i > max) ? max : i;
}

double ControllerAcro::double_constrain(const double i, const double min, const double max) {
	return (i < min) ? min : (i > max) ? max : i;
}
...
void ControllerAcro::callback_force_comp(const mavros_msgs::ActuatorControl::ConstPtr& msg_in) {
	force_compensation_ = *msg_in;
}

void ControllerAcro::callback_attitude( const mavros_msgs::AttitudeTarget::ConstPtr& msg_in ) {
	goal_att_ = *msg_in;
}

Eigen::Vector3d ControllerAcro::calc_ang_error(const Eigen::Matrix3d &R_sp, const Eigen::Matrix3d &R) {
...
void ControllerAcro::callback_mixer(const ros::TimerEvent& event) {
	...
	if(!( goal_att_.type_mask & goal_att_.IGNORE_ROLL_RATE )) {
		...
		double_constrain(control_rates_.r, controller_ang_x_.getOutputMin(), controller_ang_x_.getOutputMax());
	} else {
	...
	if(!( goal_att_.type_mask & goal_att_.IGNORE_PITCH_RATE )) {
		...
		double_constrain(control_rates_.p, controller_ang_y_.getOutputMin(), controller_ang_y_.getOutputMax());
	...
	if(!( goal_att_.type_mask & goal_att_.IGNORE_YAW_RATE )) {
		...
		double_constrain(control_rates_.y, controller_ang_z_.getOutputMin(), controller_ang_z_.getOutputMax());
	} else {
