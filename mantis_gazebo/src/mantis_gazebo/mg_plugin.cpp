...
public:
	...
	void Load( physics::ModelPtr _parent, sdf::ElementPtr /*_sdf*/ ) {
		...
		// Arming
		srv_arm_motors_ = nh_.advertiseService(
			"arm/motors", &MantisGazeboPlugin::arm_safety_srv, this );

		// PWM
		sub_rc_out_ = nh_.subscribe<mavros_msgs::OverrideRCIn>(
			"command/motor_pwm", 100, &MantisGazeboPlugin::rc_out_cb, this );
		pub_motor_velocity_ = nh_.advertise<std_msgs::Float64MultiArray>(
			"command/motor_velocity", 10 );

		// Additional Sensors
		timer_state_ = nh_.createTimer( ros::Duration( 1.0 ),
			&MantisGazeboPlugin::callback_state, this );
		...
		// Model Odom
		timer_odom_ = nh_.createTimer( ros::Duration( 0.01 ),
			&MantisGazeboPlugin::callback_odom, this );
...
private:
	int32_t int32_constrain( const int32_t i, const int32_t min,
		const int32_t max ) {
	...
	void rc_out_cb( const mavros_msgs::OverrideRCIn::ConstPtr& msg_in ) {
		...
		for ( int i = 0; i < msg_in->channels.size(); i++ ) {
			if ( safety_armed_ ) {
				...
				int pwm_raw = ( msg_in->channels[i] != 0 ) && ( msg_in->channels[i] != 65535 )
					? msg_in->channels[i]
					: 0;
				...
				// double cmd_vel = std::sqrt(norm_ref * param_motor_vel_max_ *
				// param_motor_vel_max_);

				msg_motor_velocity_.data[i] = ( cmd_vel < param_motor_vel_armed_ )
					? param_motor_vel_armed_
					: cmd_vel;
			} else {
  ...
  void statesCallback(const gazebo_msgs::ModelStates::ConstPtr& msg_in) {
          ...
          for( int i=0; i<msg_in->name.size(); i++ ) {
                  if( msg_in->name[i] == model_->GetName() ) {
                          ...
                          //The base_link frame is locked in roll and pitch with
  the world frame, but can rotate with yaw
                          tf2::Quaternion rot_l_vel;
                          tf2::Quaternion rot_a_vel;

                          //Get the rotation from the world to body frame (for
  angular measurements
                          ...
                          rot_a_vel = ( tmp_q.inverse() * tf2::Quaternion(
  msg_in->twist[i].angular.x,
                          ...
                          tf2::Vector3 body_x;
                          tf2::Vector3 body_y( r.getRow(1) );	//Use the y
  vector for yaw reference
                          ...
                          body_y.normalize();
                          r.setValue( body_x.x(), body_x.y(), body_x.z(),
  body_y.x(), body_y.y(), body_y.z(), body_z.x(), body_z.y(), body_z.z() );

                          r.getRotation( tmp_q );	//Copy the rotation back
  into the quaternion

                          rot_l_vel = ( tmp_q.inverse() * tf2::Quaternion(
  msg_in->twist[i].linear.x,
