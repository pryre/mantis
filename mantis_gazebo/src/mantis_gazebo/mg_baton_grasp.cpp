...
class MantisGazeboBatonGrasp : public ModelPlugin
{
	...
	public:
		MantisGazeboBatonGrasp() : ModelPlugin() {
		}
		...
		void Load(physics::ModelPtr _parent, sdf::ElementPtr /*_sdf*/) {
			...
			srv_do_grasp_ = nh_.advertiseService(model_->GetName() + "/do_grasp", &MantisGazeboBatonGrasp::callback_do_grasp, this);
	...
	private:
		bool callback_do_grasp( std_srvs::Trigger::Request  &req,
			...
			physics::LinkPtr link_baton = model_->GetWorld()->GetModel("baton_and_stand::baton")->GetLink("baton");
			...
			*/
			if( (link_baton != NULL) && (link_end != NULL) && (joint_grasp == NULL) ) {
				...
				joint = model_->CreateJoint("joint_baton_grasp", "fixed", link_end, link_baton);
